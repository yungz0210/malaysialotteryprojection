...
def load_data():
    ...
    for name, url in urls.items():
        # Define game-specific columns
        if name == "Toto":
            game_columns = [
              "Year", "Month", "Day", "First Prize", "Second Prize", "Third Prize", "Starter Prize 1", "Starter Prize 2", "Starter Prize 3", "Starter Prize 4", "Starter Prize 5", "Starter Prize 6", "Starter Prize 7", "Starter Prize 8", "Starter Prize 9", "Starter Prize 10", "Consolation Prize 1", "Consolation Prize 2", "Consolation Prize 3", "Consolation Prize 4", "Consolation Prize 5", "Consolation Prize 6", "Consolation Prize 7", "Consolation Prize 8", "Consolation Prize 9", "Consolation Prize 10", "DrawDay"
            ]
        else:
            game_columns = columns  # keep original columns for Magnum/Damacai
        ...
        # Normalize prize columns dynamically
...
def build_position_markov(sequences):
    # sequences is list of strings length 4 (numbers zfilled)
    ...
    for s in sequences:
        ...
        for pos in range(4):
            ...
            last_digits[pos] = d
    # convert to probs with smoothing
    pos_probs = {}
    for pos in range(4):
        mat = pos_trans[pos]
        mat += 1e-6  # tiny smoothing
        row_sums = mat.sum(axis=1, keepdims=True)
        row_sums[row_sums==0]=1.0
        pos_probs[pos] = mat / row_sums
    return pos_probs, last_digits
...
def build_full_chain_markov(sequences):
    ...
    row_sums = trans_counts.sum(axis=1, keepdims=True); row_sums[row_sums==0]=1.0
...
# Session caching for trained models to avoid retrain on small UI changes
...
def filter_df(df, year_range=None, date_range=None, drawdays=None, prize_type='All'):
    ...
    # Prize type filter
    if prize_type and prize_type != 'All':
        out = out[out[prize_type].notna() & (out[prize_type].str.strip() != '')]
...
if page=='Analytics':
    ...
    filtered = filter_df(df_game, year_range=year_range, date_range=(date_start, date_end), drawdays=drawday_choice, prize_type=prize_type)
    st.markdown('### Filter summary')
    st.write(f"Game {game_choice}  Year range {year_range}  Date range {date_start} to {date_end}  Prize {prize_type}")

    # collect numbers while preserving leading zeros
    if prize_type=='All':
        nums = filtered[PRIZE_COLS].fillna('').astype(str).values.flatten()
        nums = [x for x in nums if x and x.strip()!='']
        nums = [x.zfill(4) if x.isdigit() else x for x in nums]
    else:
        nums = filtered[prize_type].fillna('').astype(str).tolist()
        nums = [x for x in nums if x and x.strip()!='']
        nums = [x.zfill(4) if x.isdigit() else x for x in nums]
    ...
    if not nums:
        ...
        s = pd.Series(nums)
        freq = s.value_counts().reset_index(); freq.columns=['Number','Frequency']
        top10 = freq.head(10); bottom10 = freq.tail(10)
        st.subheader('Frequency Distribution')
        fig, ax = plt.subplots(figsize=(10,4))
        freq.head(50).plot(kind='bar', x='Number', y='Frequency', ax=ax)
        st.pyplot(fig)
        st.subheader('Hot Numbers (Top 10)'); st.write(top10)
        st.subheader('Cold Numbers (Bottom 10)'); st.write(bottom10)
...
# ---------- NUMBER LOOKUP ----------
    ...
    lookup_game = st.selectbox('Lookup in game', list(data.keys()), index=list(data.keys()).index(game_choice))
    df_lookup = data[lookup_game]
    filtered_lookup = filter_df(df_lookup, year_range=year_range, date_range=(date_start, date_end), drawdays=drawday_choice, prize_type='All')
        ...
        for idx, row in filtered_lookup.iterrows():
            for c in PRIZE_COLS:
                ...
                if isinstance(v,str) and v.zfill(4)==lookup_num:
                    hits.append({'draw_date': row.get('draw_date'), 'prize': c, 'DrawDay': row.get('DrawDay'), 'row_index': idx})
        if not hits:
            ...
            freq_by_prize = hits_df['prize'].value_counts().reset_index(); freq_by_prize.columns=['Prize','Count']
            st.subheader('Frequency by prize type'); st.write(freq_by_prize)
            drawday_counts = hits_df['DrawDay'].value_counts().reset_index(); drawday_counts.columns=['DrawDay','Count']
            st.subheader('Draw day distribution for this number'); st.write(drawday_counts)
...
# ---------- PREDICTION ----------
    ...
    if model_choice=='Frequency Analysis':
        ...
        if not valid_numbers:
            ...
            candidates = []
            for a in top_choices[0]:
                for b in top_choices[1]:
                    for c in top_choices[2]:
                        for d in top_choices[3]:
                            candidates.append(f"{a}{b}{c}{d}")
            candidates = candidates[:top_n]
            st.subheader('Predicted Numbers by Digit Distribution'); st.write(candidates)
        ...
        if not valid_numbers:
            ...
            for _ in range(int(num_samples)):
                ...
                s = f"{d0}{d1}{d2}{d3}"
                lp = safe_log(init_probs[d0]) + safe_log(trans_probs[d0,d1]) + safe_log(trans_probs[d1,d2]) + safe_log(trans_probs[d2,d3])
                if s not in scores or lp>scores[s]:
                    scores[s]=lp
            ranked = sorted(scores.items(), key=lambda kv: kv[1], reverse=True)[:top_n]
            out = pd.DataFrame({'Number':[n for n,_ in ranked], 'ChainScore':[float(np.exp(lp)) for _,lp in ranked]})
            st.subheader('Predicted Numbers by Markov Chain'); st.write(out)
        ...
        if not valid_numbers:
            ...
            for i in range(len(last_digits)-1):
                a=last_digits[i]; b=last_digits[i+1]; trans[a,b]+=1
            trans+=1e-6
            row_sums=trans.sum(axis=1, keepdims=True); row_sums[row_sums==0]=1.0
...
# ---------- MACHINE LEARNING ----------
        ...
        cache_key = (game_choice, str(date_start), str(date_end), tuple(sorted(drawday_choice)), prize_type, ml_method, n_lag, lstm_epochs, lstm_batch)
